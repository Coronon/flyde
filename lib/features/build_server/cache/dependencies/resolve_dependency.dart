...
/// Resolves the [dependency] of [origin], which is located within [entryDirectory], and returns
...
Future<String> resolve(
  ...
  if (RegExp(r'^.*(\/|\\).+$').hasMatch(dependency)) {
    ...
    final file = File(normalize(join(entryDirectory.path, resolved)));

    if (!await file.exists() || !isWithin(entryDirectory.path, file.path)) {
      throw ArgumentError('Cannot resolve dependency "$dependency" to an existing file.');
    }

    return SourceFile.fromFile(origin.entry, file, entryDirectory: entryDirectory).id;
  }
